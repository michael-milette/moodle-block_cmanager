...
if ($currentmode == 1) { // Make a new request.
    ...
    if (has_capability('block/cmanager:editrecord', $context)) {
        ...
        throw new \moodle_exception('cannoteditrequest', 'block_cmanager');
    }
...
class block_cmanager_courserequest_form extends moodleform {
    ...
    public function definition() {
        ...
        if ($field3status == 'enabled') {
            ...
            $mform->addRule('programmemode', get_string('request_rule2', 'block_cmanager'), 'required', null,
                    'server', false, false);
        ...
        if (false) { // This functionality has not yet been implemented.
            $mform->addElement('editor', 'summaryeditor', get_string('summary'), null, course_request::summaryeditor_options());
            $mform->addHelpButton('summaryeditor', 'coursesummary');
