...
if (isset($_GET['up'])) {
    ...
    $currentrecord = $DB->get_record('block_cmanager_formfields',
            array('id' => $currentid, 'formid' => $formid), $fields = '*', IGNORE_MULTIPLE);

    $currentposition = $currentrecord->position;

    // Record above the one being moved.
    $higherpos = ($currentposition - 1);
    $higherrecord = $DB->get_record('block_cmanager_formfields',
            array('position' => $higherpos, 'formid' => $formid), $fields = '*', IGNORE_MULTIPLE);
...
if (isset($_GET['id'])) {
    $formid = $_GET['id'];
    $currentrecord = $DB->get_record('block_cmanager_config', array('id' => $formid));
    $formname = $currentrecord->value;
...
$htmloutput = '
    ...
    <script>
        ...
        function setLangStrings(lang_dropdownTxt,
            lang_radioTxt,
            lang_textAreaTxt,
            lang_textFieldTxt,
            lang_leftTxt,
        ...
        setLangStrings("' . get_string('formBuilder_dropdownTxt', 'block_cmanager') . '","'
                . get_string('formBuilder_radioTxt', 'block_cmanager') . '","'
                . get_string('formBuilder_textAreaTxt', 'block_cmanager') . '","'
                . get_string('formBuilder_textFieldTxt', 'block_cmanager') . '","'
                . get_string('formBuilder_leftTxt', 'block_cmanager') . '","'
                . get_string('formBuilder_saveTxt', 'block_cmanager') . '","'
                . get_string('formBuilder_addedItemsTxt', 'block_cmanager') . '","'
                . get_string('formBuilder_addItemBtnTxt', 'block_cmanager') . '")
    </script>

    <a class="btn btn-default" href="form_builder.php"><img src="../icons/back.png"/> '
            . get_string('back', 'block_cmanager') . '
    </a>
    ...
    <p class="mt-3">
        <a class="btn btn-default" href="preview.php?id=' . $formid . '">'
                . get_string('formBuilder_previewForm', 'block_cmanager') . '</a>
        <a class="btn btn-default" href="../cmanager_admin.php">' . get_string('formBuilder_returntoCM', 'block_cmanager') . '</a>
    </p>
    ';
    $htmloutput .= generategenericpop('saved',
...
/**
 * Save the form field status as either a required field or an optional field. Display a modal after saving.
 */
...
function recreateTextField(uniqueId, lefttext, requiredfieldvalue) {
    ...
    var icons;
    if(numberoffields == 1) {
        icons = '<a href="page2.php?id=' + formid + '&del=' + uniqueId + '"><i class="icon fa fa-trash fa-fw " title="<?php
                echo get_string('delete'); ?>" aria-label="<?php echo get_string('delete'); ?>"></i></a>';
    } else {
        if(num == 1){
            icons = '<a href="page2.php?id=' + formid + '&down=' + uniqueId + '">
                <i class="icon fa fa-arrow-down fa-fw " title="<?php echo get_string('movedown'); ?>" aria-label="<?php
                    echo get_string('movedown'); ?>"></i></a>'+
                '<a href="page2.php?id=' + formid + '&del=' + uniqueId + '"><i class="icon fa fa-trash fa-fw " title="<?php
                    echo get_string('delete'); ?>" aria-label="<?php echo get_string('delete'); ?>"></i></a>';
        } else if(movedownEnabled == 0){
            icons = '<a href="page2.php?id=' + formid + '&up=' + uniqueId + '"><i class="icon fa fa-arrow-up fa-fw " title="<?php
                    echo get_string('moveup'); ?>" aria-label="<?php echo get_string('moveup'); ?>"></i></a>'+
                '<a href="page2.php?id=' + formid + '&del=' + uniqueId + '"><i class="icon fa fa-trash fa-fw " title="<?php
                    echo get_string('delete'); ?>" aria-label="<?php echo get_string('delete'); ?>"></i></a> ';
        } else {
            icons = '<a href="page2.php?id=' + formid + '&up=' + uniqueId + '"><i class="icon fa fa-arrow-up fa-fw " title="<?php
                    echo get_string('moveup'); ?>" aria-label="<?php echo get_string('moveup'); ?>"></i></a>'+
                '<a href="page2.php?id=' + formid + '&down=' + uniqueId + '"><i class="icon fa fa-arrow-down fa-fw " title="<?php
                    echo get_string('movedown'); ?>" aria-label="<?php echo get_string('movedown'); ?>"></i></a>'+
                '<a href="page2.php?id=' + formid + '&del=' + uniqueId + '"><i class="icon fa fa-trash fa-fw " title="<?php
                    echo get_string('delete'); ?>" aria-label="<?php echo get_string('delete'); ?>"></i></a>';
        }
    }
    newdiv.innerHTML = '<div class="row">'+
        ...
        '<input type="text" id = "x'+uniqueId +'" size="30" value="' + lefttext+ '" onfocus="enableSave(\''+uniqueId+'_savebtn\');">
                </input>' +
        ...
        '<option '+selectedText+' value="1"> <?php echo get_string('required_field', 'block_cmanager'); ?></option>  ' +
                '</select>'+
        '<input type="button" value="'+saveTxt+'" disabled="disabled" id="'+uniqueId+'_savebtn"
                onclick="saveFieldValue(' + uniqueId+')"/>'+
        '</div>'+
...
/**
 * This function is used to take the value from the textfield beside the "Add New item" button on dropdown menus.
 *
 * @return void
 */
...
function recreateTextArea(uniqueId, lefttext, requiredfieldvalue){
    ...
    var icons;
    if(numberoffields == 1){
        icons = '<a href="page2.php?id=' + formid + '&del=' + uniqueId + '">
                <i class="icon fa fa-trash fa-fw " title="<?php echo get_string('delete'); ?>" aria-label="<?php
                echo get_string('delete'); ?>"></i></a>';
    } else {
        if(num == 1) {
            icons = '<a href="page2.php?id=' + formid + '&down=' + uniqueId + '"><i class="icon fa fa-arrow-down fa-fw "
                    title="<?php echo get_string('movedown'); ?>" aria-label="<?php echo get_string('down'); ?>"></i></a>'+
                '<a href="page2.php?id=' + formid + '&del=' + uniqueId + '"><i class="icon fa fa-trash fa-fw " title="<?php
                    echo get_string('delete'); ?>" aria-label="<?php echo get_string('delete'); ?>"></i></a>';
        } else if(movedownEnabled == 0){
            icons = '<a href="page2.php?id=' + formid + '&up=' + uniqueId + '"><i class="icon fa fa-arrow-up fa-fw "
                    title="<?php echo get_string('moveup'); ?>" aria-label="<?php echo get_string('moveup'); ?>"></i></a>'+
                '<a href="page2.php?id=' + formid + '&del=' + uniqueId + '"><i class="icon fa fa-trash fa-fw " title="<?php
                    echo get_string('delete'); ?>" aria-label="<?php echo get_string('delete'); ?>"></i></a>';
        } else {
            icons = '<a href="page2.php?id=' + formid + '&up=' + uniqueId + '"><i class="icon fa fa-arrow-up fa-fw "
    ...
    newdiv.innerHTML = '<div class="row">'+
        ...
        ' <input type="text" id = "x'+uniqueId +'" size="30" value="' + lefttext+ '"
                onfocus="enableSave(\''+uniqueId+'_savebtn\');"></input>'+
        ' <select id = "optional_'+uniqueId+'" onchange="saveOptionalStatus('+uniqueId+')">  ' +
        '   <option value="0"> <?php echo get_string('optional_field', 'block_cmanager'); ?> </option>   ' +
        '   <option '+selectedText+' value="1"> <?php echo get_string('required_field', 'block_cmanager'); ?></option>  ' +
        ' </select>'+
        ' <input type="button" value="'+saveTxt+'"
                disabled="disabled" id="'+uniqueId+'_savebtn" onclick="saveFieldValue(' + uniqueId+')"/>'+
        '</div>'+
        '<div class="col-sm-2">'+icons+
        '</div>'+
        '</div>';
...
function recreateDropdown(uniqueId, lefttext, requiredfieldvalue) {
    ...
    $.post("ajax_functions.php", { id: uniqueId, type: 'getdropdownvalues'},
        function(data) {
            ...
            var icons;
            if(numberoffields == 1){
                icons ='<a href="page2.php?id=' + formid + '&t=drop&del=' + uniqueId + '">
                        <i class="icon fa fa-trash fa-fw " title="<?php echo get_string('delete'); ?>" aria-label="<?php echo
                        get_string('delete'); ?>"></i></a>';
            } else {
                if(num == 1){
                    icons ='<a href="page2.php?id=' + formid + '&down=' + uniqueId + '"><i class="icon fa fa-arrow-down fa-fw "
                            title="<?php echo get_string('movedown'); ?>" aria-label="<?php echo get_string('down'); ?>"></i></a>'+
                        '<a href="page2.php?id=' + formid + '&t=drop&del=' + uniqueId + '"><i class="icon fa fa-trash fa-fw "
                            title="<?php echo get_string('delete'); ?>" aria-label="<?php echo get_string('delete'); ?>"></i></a>';
                }
                else if(movedownEnabled == 0){
                    icons ='<a href="page2.php?id=' + formid + '&up=' + uniqueId + '"><i class="icon fa fa-arrow-up fa-fw "
                        title="<?php echo get_string('moveup'); ?>" aria-label="<?php echo get_string('moveup'); ?>"></i></a>'+
                    ...
                    icons ='<a href="page2.php?id=' + formid + '&up=' + uniqueId + '"><i class="icon fa fa-arrow-up fa-fw "
                        title="<?php echo get_string('moveup'); ?>" aria-label="<?php echo get_string('moveup'); ?>"></i></a>'+
                        '<a href="page2.php?id=' + formid + '&down=' + uniqueId + '"><i class="icon fa fa-arrow-down fa-fw "
                        title="<?php echo get_string('movedown'); ?>" aria-label="<?php echo get_string('down'); ?>"></i></a>'+
                        '<a href="page2.php?id=' + formid + '&t=drop&del=' + uniqueId + '"><i class="icon fa fa-trash fa-fw "
                        title="<?php echo get_string('delete'); ?>" aria-label="<?php echo get_string('delete'); ?>"></i></a>';
                }
            ...
            newdiv.innerHTML = '<div class="row">'+
                ...
                '</div>'+
                '</div>'+
                ' <input type="text" id="newitem'+uniqueId +'"> <input type="button" name="submitbutton"
                        value="'+addItemBtnTxt+'" onclick="addNewItem('+ uniqueId +');">'+
                '<p>'+addedItemsTxt+':</p>'+
                fieldsInHTML;
        }
    );
...
function recreateRadio(uniqueId, lefttext, requiredfieldvalue){
    ...
    $.post("ajax_functions.php", { id: uniqueId, type: 'getdropdownvalues'},
        ...
        function(data) {
            ...
            var icons;
            if (numberoffields == 1) {
                icons = '<a href="page2.php?id=' + formid + '&del=' + uniqueId + '"><i class="icon fa fa-trash fa-fw "
                        title="<?php echo get_string('delete'); ?>" aria-label="<?php echo get_string('delete'); ?>"></i></a>';
            } else {
                if(num == 1){
                    icons = '<a href="page2.php?id=' + formid + '&down=' + uniqueId + '"><i class="icon fa fa-arrow-down fa-fw "
                            title="<?php echo get_string('movedown'); ?>" aria-label="<?php echo get_string('down'); ?>"></i></a>'+
                        '<a href="page2.php?id=' + formid + '&del=' + uniqueId + '"><i class="icon fa fa-trash fa-fw "
                            title="<?php echo get_string('delete'); ?>" aria-label="<?php echo get_string('delete'); ?>"></i></a>';
                } else if(movedownEnabled == 0){
                    icons = '<a href="page2.php?id=' + formid + '&up=' + uniqueId + '"><i class="icon fa fa-arrow-up fa-fw "
                            title="<?php echo get_string('moveup'); ?>" aria-label="<?php echo get_string('moveup'); ?>"></i></a>'+
...
/**
 * Saves the text field data to the database by passing the field id.
 *
 * @return void
 */
...
foreach ($formfields as $field) {
    ...
    switch ($field->type) {
        case 'textfield':
            echo "<script>recreateTextField('". $field->id ."', '". $lt ."', '". $field->reqfield ."');</script>";
            break;
        case 'textarea':
            echo "<script>recreateTextArea('". $field->id ."', '". $lt ."', '". $field->reqfield ."');</script>";
            break;
        case 'dropdown':
            echo "<script>recreateDropdown('". $field->id ."', '". $lt ."', '". $field->reqfield ."');</script>";
            break;
        case 'radio':
            echo "<script>recreateRadio('". $field->id ."', '". $lt ."', '". $field->reqfield ."');</script>";
    }
