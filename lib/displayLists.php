...
// This displayLists.php is used to display the various different representations of queues etc.
...
function block_cmanager_display_admin_list($pendinglist, $includerightpanel, $includeleftcheckbox,
    ...
    foreach ($pendinglist as $rec) {
        ...
        $lecturerhtml = '';
        $reqvalues = $rec->req_values;

        if (!empty($reqvalues)) {
            if ($course = $DB->get_record("course", array("id" => $reqvalues))) {
                // If the course doesn't exist anymore, just let the process continue. Otherwise, start the process.
                ...
                <div class="custom-control custom-checkbox mr-1">
                    <h2 class="border-top mt-5">
                        <input type="checkbox" id="' . $rec->id .
                            '" class="bulk-action-checkbox custom-control-input mt-3" name="groupedcheck"
                                onClick="addIdToList(' . $rec->id .
                            ')" value="' . $rec->id . '" ' . $disabledhtml . '/>
                        <label for="' . $rec->id . '" class="custom-control-label">'
                                . get_string('Request', 'block_cmanager') . ' ID #' . $rec->id . ': '
                                . format_string($rec->modcode) . '</label>
                    </h2>
                </div>';
        } else {
            $outputhtml .= '
            <h2 class="border-top mt-5">' . get_string('Request', 'block_cmanager') . ' ID #' . $rec->id . ': '
                    . format_string($rec->modcode) . '</h2>
            </div>';
        }
        // Additional Controls.
            ...
            if ($rightpaneltype == 'admin_queue') {
                $outputhtml .= '
                    <div class="btn-group" padding-bottom:6.5px" id="existingrequesticons">
                        <a class="btn btn-default" href="#" onclick="quickApproveConfirm(' . $rec->id . ',\''
                            . get_string('quickapprove_desc', 'block_cmanager') . '\')">
                            <img src="icons/list/quick.png"/>&nbsp;' . get_string('quickapprove', 'block_cmanager') . '</a>
                        <a class="btn btn-default" href="admin/approve_course.php?id=' . $rec->id . '">
                            <img src="icons/list/approve.png">&nbsp;' . get_string('approve', 'block_cmanager') . '</a>
                        <a class="btn btn-default" href="admin/deny_course.php?id=' . $rec->id . '">
                            <img src="icons/list/deny.png"/>&nbsp;' . get_string('deny', 'block_cmanager') . '</a>
                        <a class="btn btn-default" href="course_request.php?mode=2&edit=' . $rec->id . '">
                            <img src="icons/list/edit.png"/>&nbsp;' . get_string('edit', 'block_cmanager') . '</a>
                        <a class="btn btn-default" href="#" onclick="cancelConfirm(' . $rec->id . ',\''
                            . get_string('configure_delete', 'block_cmanager') . '\')" href="#">
                            <img src="icons/list/delete.png"/>&nbsp;' . get_string('delete', 'block_cmanager') . '</a>
                        <a class="btn btn-default" href="comment.php?type=adminq&id=' . $rec->id . '">
                            <img src="icons/list/comment.png"/>&nbsp;' . get_string('addviewcomments', 'block_cmanager') . '</a>
                    </div>
                ';

            } else if ($rightpaneltype == 'admin_arch') { // For cmanager_admin_arch.php.
                $outputhtml .= '
                <div class="btn-group" padding-bottom:6.5px">
                    <a class="btn btn-default" onclick="cancelConfirm(' . $rec->id . ', \'delete\')" href="#">
                        <img src="icons/list/delete.png"/>&nbsp;' . get_string('delete', 'block_cmanager') . '</a>
                    <a class="btn btn-default" href="comment.php?type=adminarch&id=' . $rec->id . '">
                        <img src="icons/list/comment.png"/>&nbsp;' . get_string('addviewcomments', 'block_cmanager') . '</a>
                </div>
            ';
            } else if ($rightpaneltype == 'user_manager') { // For module_manager.php.

                $outputhtml .= '
                    <div class="btn-group" padding-bottom:6.5px" id="existingrequesticons">
                        <a class="btn btn-default" href="view_summary.php?id=' . $rec->id . '">
                            <img src="icons/list/open.png"/>&nbsp;' . get_string('view', 'block_cmanager') . '</a>
                        <a class="btn btn-default" href="course_request.php?mode=2&edit=' . $rec->id . '">
                            <img src="icons/list/edit.png"/>&nbsp;' . get_string('edit', 'block_cmanager') . '</a>
                        <a class="btn btn-default" onclick="cancelConfirm(' . $rec->id . ',\''
                            . get_string('cmanagerConfirmCancel', 'block_cmanager') . '\')" href="#">
                            <img src="icons/list/deny.png"/>&nbsp;' . get_string('cancel', 'block_cmanager') . '</a>
                        <a class="btn btn-default" href="comment.php?type=userq&id=' . $rec->id . '">
                            <img src="icons/list/comment.png"/>&nbsp;' . get_string('addviewcomments', 'block_cmanager') . '</a>
                    </div>';
            } else if ($rightpaneltype == 'user_history') { // For module_manager_history.php.
                $outputhtml .= '
                    <div class="btn-group" padding-bottom:5px">
                        <a class="btn btn-default" href="view_summary.php?id=' . $rec->id . '">
                            <img src="icons/list/open.png"/>&nbsp;' . get_string('view', 'block_cmanager') . '</a>
                        <a class="btn btn-default" href="comment.php?type=userarch&id='
                            . $rec->id . '"><img src="icons/list/comment.png"/>&nbsp;'
                            . get_string('addviewcomments', 'block_cmanager') . '</a>
                    </div>';
            }
        }
        // END admin controls.
        ...
        $outputhtml .= '
            ...
            <tr>
                <th>' . format_string($page1fieldname1) . ':</th>
                <td>' . format_string($rec->modcode) . '</td>
            </tr>

            <tr>
                <th>' . format_string($page1fieldname2) . ':</th>
